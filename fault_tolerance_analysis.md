...
> **Ventana de falla:** caída entre `_flush_batch_to_disk` y `_save_checkpoint` — puede causar envíos duplicados.
...
> **Ventana de falla:** caída entre `_send_output` y `_save_checkpoint` — puede causar EOF duplicado downstream.
...
> **Ventana de falla:** caída entre `_send_batch` y `os.remove` en `_load_checkpoint` — puede causar envíos duplicados.
...
> **Ventana de falla:** caída entre `_try_send_eof` y `_save_checkpoint` — puede causar EOF duplicado downstream.
...
> **Ventana de falla:** caída dentro de `_try_send_eof` antes de `_save_checkpoint` — puede causar EOF duplicado downstream.
...
> **Ventana de falla:** caída entre `_flush_first_batch`/`_flush_second_batch` y `_save_checkpoint` — puede causar envíos duplicados.
...
> **Ventana de falla:** caída entre `_flush_all_batches`/envío downstream y `_save_checkpoint` — puede causar duplicado de filas y EOF downstream (solo para `counter > 1` o rama `else`).
...
> **Ventana de falla:** caída entre `output_queue.send` y `_save_checkpoint` — puede causar envíos duplicados.
